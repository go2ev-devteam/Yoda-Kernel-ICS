...
static void p3_s5k5ccgx_power_off(void)
{
	...
	if (system_rev < 14)
		...
		msleep(200);
		pr_debug(KERN_DEBUG "p3_s5k5ccgx_power_off\
...
static int P3_s5k5ccgx_flash(int enable)
{
	...
	pr_debug(KERN_DEBUG "========== flash enable = %d\n", enable);
...
static int P3_s5k5ccgx_af_assist(int enable)
{
	...
	pr_debug(KERN_DEBUG "========== flash af_assist =========== %d\n",\
			enable);
#if 0
	gpio_set_value(GPIO_CAM_FLASH_EN, enable);
	if (enable) {
...
{
	pr_debug(KERN_DEBUG "TDMB tdmb_gpio_on\n");
...
{
	pr_debug(KERN_DEBUG "TDMB tdmb_gpio_off\n");
...
{
	pr_debug(KERN_DEBUG "TDMB tdmb_dev_init\n");
